...
- [ ] написать тесты `format_duration`: 0 → `00:00:00`, 59.9с → `00:00:59`, 60с → `00:01:00`, 3599с → `00:59:59`, 3600с → `01:00:00`, 24ч → `24:00:00`, 100ч+ → `103:25:10`-кейс, отрицательное → `00:00:00`, дробные секунды — усечение
- [ ] написать тесты `_parse_ts`: `Z`-суффикс, `+00:00`, смещение зоны, naive → UTC, мусор/None/пусто → None, мс сохраняются
- [ ] прогнать — красный
- [ ] реализовать обе функции рядом с `format_tokens`
- [ ] прогнать — зелёный, до задачи 2
...
- [ ] написать тесты: пустой список → 0; один интервал; пересечение; вложенность; смежность склеивается (`[0,10]+[10,20]` → 20); несортированный вход; вырожденные `e<=s` отбрасываются; «дырки» — интервал с вырезанной серединой суммируется из кусков
- [ ] прогнать — красный
- [ ] реализовать `union_work`
- [ ] прогнать — зелёный, до задачи 3
