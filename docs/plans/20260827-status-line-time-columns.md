...
- [ ] проверить требования Overview: три колонки, union-семантика, ожидание агентов = работа, QA-паузы, live-now, формат, распределение по строкам
- [ ] проверить края: нет ts → пустые ячейки; до-апгрейдные кэши перечитываются; пустой stdin / битый jsonl не падают
- [ ] полный прогон: `python3 -m pytest tests/ -v`
- [ ] убедиться, что старые тесты не деградировали (обновлены там, где менялся контракт)

### Task 8: [Final] Update documentation

- [ ] обновить `README.md`: примеры вывода с тремя колонками (оба режима), семантика work/wait/total (union, QA-паузы, live-now), новые поля кэшей, актуальное число тестов
