...
- [ ] split-сообщение считается один раз в обоих сканах; записи без id — как раньше
- [ ] content-обработка (tool_use positions, QA-паузы, сегментер) не под гейтом — покрыто тестом Task 1
- [ ] существующие тесты не потребовали правок ожиданий (регрессия no-op подтверждена)
- [ ] полный прогон: `python -m pytest tests/ -v` — все зелёные
